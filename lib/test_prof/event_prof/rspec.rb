...
module TestProf
  module EventProf
    class RSpecListener # :nodoc:
      ...
      def initialize
        @profilers = EventProf.build
      end

      def example_group_started(notification)
        return unless notification.group.top_level?
        @profilers.each { |profiler| profiler.group_started notification.group }
      end

      def example_group_finished(notification)
        return unless notification.group.top_level?
        @profilers.each { |profiler| profiler.group_finished notification.group }
      end

      def example_started(notification)
        @profilers.each { |profiler| profiler.example_started notification.example }
      end

      def example_finished(notification)
        @profilers.each { |profiler| profiler.example_finished notification.example }
      end

      def stop(notification)
        @profilers.each { |profiler| profiler.suite_finished }
      end
